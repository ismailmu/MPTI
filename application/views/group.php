	...
	<body>
									...
									<table class="table table-striped table-bordered table-hover" id="tbl">
										...
										<tbody>
											...
											foreach ($grid as $key => $value) {
												...
												echo "<td>$value->group_code</td>";
												echo "<td>$value->group_name</td>";
												$checked = ($value->is_user == 1) ? "checked='checked'" : "";
												echo "<td class='fCtr'><input $checked type='checkbox' disabled='disabled'/></td>";
												$checked = ($value->is_active == 1) ? "checked='checked'" : "";
												echo "<td class='fCtr'><input $checked type='checkbox' disabled='disabled'/></td>";
												$path=base_url();
												echo "<td class='fCtr'><button type='button' onclick=\"gotoEdit('$value->group_code')\" class='btn btn-outline btn-primary btn-xs'><i class='fa fa-edit fa-fw'></i>Edit</button></td>";
												echo "</tr>";
		...
		<script>
			...
			function gotoAdd(path) {
				window.location.href = path + 'index.php/group/add';
			}
			function gotoEdit(id) {
				window.location.href = '<?php echo base_url(); ?>index.php/group/edit/'+id;
			}
		</script>
	</body>
